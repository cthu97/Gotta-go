...
$(document).ready(function () {
  ...
  const updateUserLocation = () => {
    window.map
      ...
      .on("locationerror", function (e) {
        console.log(
          "location blocked, user lat:",
          window.user.latitude,
          "long:",
          window.user.longitude
        );
        if (userIsLoggedIn()) {
          makeUserPin(
            window.user.latitude,
            window.user.longitude,
            `<p>Your last location was here</p>`
          );
        } else {
          makeUserPin(
            window.user.latitude,
            window.user.longitude,
            `<p>No location found, default to Vancouver City Hall</p>`
          );
        }
      });
  };
  ...
  const setUserLocation = (user) => {
    $.ajax({
      ...
      success: function () {
        console.log(
          `user location Successfully Patched! user lat: ${user.latitude}, user long:${user.longitude}`
        );
      },
  ...
  const getMapNearUserLocation = () => {
    ...
    $(".fav-map-div").empty();
    const $div = $(
      `<div><h3>Nearby Maps</h3><div class='nearbyMapButtons'></div></div>`
    );
    $(".nearbyMapButtons").empty();
    $div.appendTo($(".sidebar"));
    $.get(
      `/api/maps/${window.user.latitude}/${window.user.longitude}`,
      (obj) => {
  ...
  const renderPins = (map) => {
    ...
    $.get(`/api/mapPins/${map_id}`, (obj) => {
      if (userIsLoggedIn()) {
        ...
        const $addtoFave = $(
          `<div class="addFave" id="${map_id}">Add Map to Favourites 👍</div>`
        );
        $addtoFave.appendTo($sidebar);
      }
      const $h3 = $(
        `<div class='pins-container'><h3>Pins for ${map_name}</h3></div><br>`
      );
      $h3.appendTo($sidebar);
      ...
      for (let i = 0; i < obj.length; i++) {
        ...
        const pinButton = $(
          `<div class='pinButtons'><div>${title}</div> ${distance}m away </div>`
        );
        $(pinButton).attr("id", id);
        pinButton.appendTo($h3);
        if (loggedIn) {
          const deleteButton = $(
            `<div class="deleteButtons"><button>❌ delete ❌</button></div><br>`
          );
      ...
      if (loggedIn) {
        ...
        const $createPin = $(
          `<div><button class="createPin" id="${map_id}">create pin</button></div>`
        );
  ...
  $(".sidebar").on("click", ".addFave", function () {
    ...
    $.post("/api/faveMaps/", { map_id: map_id, user_id: window.user.id }).then(
      () => {
        $(".fav-map-div").empty();
        $(".nearByMap").empty();
        $(".sidebar footer").empty();
  ...
  $(".sidebar").on("click", ".map-button", function () {
    ...
    $.getJSON(`http://localhost:8080/api/maps/${buttonID}`, function (result) {
      ...
      $(".sidebar").on("click", ".pinButtons", function () {
        ...
        $.getJSON(
          `http://localhost:8080/api/pins/${buttonID}`,
          function (result) {
            console.log("result", result);
            ...
            window.mapLayers.eachLayer(function (layer) {
              layer.eachLayer(function (marker) {
                if (marker.pin_id == buttonID) {
                  ...
                  const $title = $("<header>", { class: "pin_title" }).text(
                    pin.title
                  );
                  const $img = $("<img>", { class: "image" }).attr(
                    "src",
                    pin.image_url
                  );
                  const $description = $("<p>", { class: "write_up" }).text(
                    pin.description
                  );
  ...
  $(".sidebar").on("click", ".nearByMaps", function () {
    ...
    const $nearByBackButton = $(
      `<div class="nearByBackButton"><button>back</button></div>`
    );
  ...
  $("div.pin_container").on("submit", ".pinForm", function (event) {
    ...
    // get map_id
    $.post(`/api/pins/`, data).then((obj) => {
      const pinID = obj.pin[0].id;
      const pin = obj.pin[0];
      console.log("line 323", obj, "dafs", obj.pin[0].id);
      // add to database
      $.post(`/api/mapPins/${obj.map_id}`, { pinID }).then((object) => {
        window.mapLayers.eachLayer(function (layer) {
  ...
  const renderNavLoggedIn = () => {
    ...
    $.get(`/api/users/${user_id}`, (obj) => {
      ...
      // make nearby button
      const $nearbyMapButton = $(
        '<div class="nearByMaps"><button class="nearByButton">Nearby Maps</button></div>'
      );
      $nearbyMapButton.appendTo($(".sidebar"));

      // make create map button
      const $createButton = $(
        '<footer><button class="createMap">Create Map</button></footer>'
      );
    ...
    $form.submit((event) => {
      ...
      if (
        ...
        isNaN(parseFloat($.trim($("#latitude").val()))) ||
        isNaN(parseFloat($.trim($("#longitude").val())))
      ) {
        return $("span")
          .text("Please be mature and input proper latitude or longitude!")
          .show()
          .fadeOut(4000);
      }
      const data = $form.serialize();
      $.post(`/api/maps/`, data).then(() => {
        console.log(data);
        $.get(`/api/users/${user_id}`, (obj) => {
          renderNav();
        });
    ...
    $.get(`/api/faveMaps/${user_id}`, (obj) => {
      ...
      for (let i = 0; i < obj.length; i++) {
        ...
        const $faveMapButton = $(
          `<div>${name}<br>created by: ${username}</div><br>`
        );
